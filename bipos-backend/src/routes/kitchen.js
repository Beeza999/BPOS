...
kitchenRouter.patch('/tickets/:itemId/status', requireAuth, allowRoles('OWNER', 'ADMIN', 'KITCHEN'), async (req, res, next) => {
  try {
    ...
    if (!['NEW', 'ACCEPTED', 'COOKING', 'READY', 'SERVED', 'CANCELLED'].includes(status)) throw httpError(400, 'Invalid status');
    const updated = await prisma.orderItem.update({ where: { id: req.params.itemId }, data: { status }, include: { order: { include: { table: true, items: true } } } });
    const siblings = await prisma.orderItem.findMany({ where: { orderId: updated.orderId } });
    const orderStatus = siblings.every((i) => (i.id === updated.id ? status === 'READY' : i.status === 'READY')) ? 'READY' : status;
    const order = await prisma.order.update({ where: { id: updated.orderId }, data: { status: orderStatus }, include: { table: true, items: true } });
    await logAudit(req, { action: 'KITCHEN_TICKET_STATUS_CHANGED', targetType: 'OrderItem', targetId: updated.id, branchId: order.branchId, metadata: { status, orderId: order.id } });
